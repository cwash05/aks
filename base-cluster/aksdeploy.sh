...
azMonWorkspaceId=$(az deployment group show  -g $aksResourceGroupName -n ${template/.bicep} --query properties.outputs.azMonWorkspaceId.value -otsv)
grafanaDashboardId=$(az deployment group show  -g $aksResourceGroupName -n ${template/.bicep}  --query properties.outputs.grafanaDashboardId.value -otsv)
...
# az aks update --enable-azuremonitormetrics -n $aksName -g $aksResourceGroupName --azure-monitor-workspace-resource-id $azMonWorkspaceId  --grafana-resource-id $grafanaDashboardId #--ksm-metric-labels-allow-list "namespaces=[*],pod=[*],service=[*],node=[*]" --ksm-metric-annotations-allow-list "os_image=[kubernetes.io/linux,kubernetes.io/windows]"
...
adminpw=$(az deployment group show  -g $aksResourceGroupName -n ${template/.bicep} --query properties.outputs.adminpw.value -otsv)
  ...
  # Deploy the Bicep monitoring template
  # echo "Deploying [$graftemplate] Bicep monitoring template..."
  # az deployment group create \
  #   --resource-group $aksResourceGroupName \
  #   --name 'GrafProDeploy' \
  #   --only-show-errors \
  #   --template-file $graftemplate \
  #   --parameters azureMonitorWorkspaceResourceId=$azMonWorkspaceId \
  #   azureMonitorWorkspaceLocation=$azureMonitorLocation \
  #   clusterResourceId=$aksClusterId \
  #   clusterLocation=$location \
  #   grafanaResourceId=$grafanaDashboardId \
